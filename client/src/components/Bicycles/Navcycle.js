...
function NavBicycle({ name, ...props }) {
  ...
  return (
    <div>
      <Navbar expand="lg" style={navcolor}>
        {/* <Navbar expand="lg" bg='transparent' className="fixed-top"> */}
        <Container>
          <Navbar.Brand className='d-flex'><Image src='../images/logo.png' className='logo1 mt-1'></Image></Navbar.Brand>
          <Navbar.Toggle aria-controls="basic-navbar-nav" />
          <Navbar.Collapse id="basic-navbar-nav">
            <Nav className="mx-auto p-2">
            <Nav.Link className='text-white ms-2 navhome'><Link to="/Index" className='navhome text-white'>HOME</Link></Nav.Link>
                <Nav.Link to="/Navcycle" className='text-white ms-2 navhome'><Link to="/Navcycle" className='navhome text-white'>BICYCLES</Link></Nav.Link>
                <Nav.Link className='text-white ms-2 navhome'><Link to="/Navaccessories" className='navhome text-white'>ACCESORIES</Link></Nav.Link>
                <Nav.Link className='text-white ms-2 navhome'><Link to="/Profile" className='navhome text-white'>PROFILE</Link></Nav.Link>
                 <Nav.Link className='text-white ms-2 navhome'><Link to="/Orderslist" className='navhome text-white'>ORDERS</Link></Nav.Link>
                {/*<Nav.Link className='text-white ms-2 navhome'><Link to="/Contact" className='navhome text-white'>CONTACT</Link></Nav.Link> */}
            </Nav>
            <Nav>
                <div className='cart-icon-container' onClick={handleShow}>
                  <i className="fas fa-shopping-cart text-white" style={{ cursor: "pointer" }}></i>
                  <span className='navcart ms-2 fw-bold d-flex d-none d-lg-block'>{fetchcart.length}</span>
                  {/* <span className='navcart ms-2 fw-bold d-flex d-none d-lg-block'>{cartCount}</span> */}
                          ...
                          <Row>
                            <div className='d-flex justify-content-between offcontent'>
                             <Link className='text-decoration-none text-dark'><span className='fw-bold'>{cart.joinedCart[0].productname}</span></Link> 
                              <span className='rounded-5 pe-3'  onClick={() => handleDelete(cart._id)}><i class="fa fa-times-circle-o"></i></span>
                            </div>
                            <div>
